...
const orderValidationSchema = z.object({
   ...
   orderWeight: z.number().min(0.5, "Order weight is required, Should be greater than 0").refine(val => Number(val) >= 0.5),
   ...
   orderBoxHeight: z.number().min(0.5, "Order box height is required").refine(val => Number(val) >= 0.5),
   orderBoxWidth: z.number().min(0.5, "Order box width is required").refine(val => Number(val) >= 0.5),
   orderBoxLength: z.number().min(0.5, "Order box length is required").refine(val => Number(val) >= 0.5),
